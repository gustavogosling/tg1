...
e=`bc <<< "scale=1; $cont + $1" | awk '{printf "%.1f", ($1 / 10)}'`
w=`bc <<< "scale=1; $cont" | awk '{printf "%.1f", ($1 / 10)}'`
ngr=`bc <<< "scale=1; ($n + 0.1) * 10" | awk '{printf "%.1f", ($1 / 10)}'`
sgr=`bc <<< "scale=1; ($s - 0.1) * 10" | awk '{printf "%.1f", ($1 / 10)}'`
egr=`bc <<< "scale=1; ($e + 0.1) * 10" | awk '{printf "%.1f", ($1 / 10)}'`
wgr=`bc <<< "scale=1; ($w - 0.1) * 10" | awk '{printf "%.1f", ($1 / 10)}'`	

# cria a primeira grade de 1x1
nome1=`awk -v w="$w" -v e="$e" -v s="$s" -v n="$n" 'BEGIN {printf "OP_%.1f_%.1f_%.1f_%.1f", w, e, s, n}'`
gmt grdcut -R$wgr/$egr/$sgr/$ngr $8 -G"$nome1".grd
bash processamento/derivadas_paralel.sh $2 $3 $4 $nc "$nome1" opgrd_"$nome1"
...
nome2=`awk -v w="$w" -v e="$e" -v s="$s" -v n="$n" 'BEGIN {printf "OP_%.1f_%.1f_%.1f_%.1f", w, e, s, n}'`
gmt grdcut -R$wgr/$egr/$sgr/$ngr $8 -G"$nome2".grd
bash processamento/derivadas_paralel.sh $2 $3 $4 $nc "$nome2" opgrd_"$nome2"
gmt grdcut -R$w/$e/$s/$n opgrd_"$nome2".grd -Gopgrd_"$nome2".grd

# cola as grades
nomelinha=`awk -v s="$s" -v n="$n" 'BEGIN {printf "LI_%.1f_%.1f", s, n}'`
...
do
	...
	nome=`awk -v w="$w" -v e="$e" -v s="$s" -v n="$n" 'BEGIN {printf "OP_%.1f_%.1f_%.1f_%.1f", w, e, s, n}'`
	gmt grdcut -R$wgr/$egr/$sgr/$ngr $8 -G"$nome".grd
	bash processamento/derivadas_paralel.sh $2 $3 $4 $nc "$nome"
